...
function Row({ to, label, page, bold }) {
  return (
    <div className="flex flex-wrap items-baseline gap-x-2">
      <Link
        to={to}
        className={`min-w-0 hover:text-brand-blue transition uppercase ${
...
export default function TocPage() {
  ...
  return (
    <div className="flex flex-col items-center gap-6 py-8 px-4">
      ...
      <PrevNextNav
        prev={{ to: '/ficha-catalografica', title: 'Ficha catalográfica' }}
        next={firstChapter ? { to: `/secao/${firstChapter.slug}`, title: firstChapter.title } : null}
      />
    </div>
  );
}
