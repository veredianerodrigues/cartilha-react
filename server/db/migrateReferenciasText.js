...
// Migra a lista de Referências pro banco como blocks editáveis (add/mover/
// remover pelo admin). Cada block usa `heading` como id ESTÁVEL da
// referência — não é a posição de exibição, é permanente, nunca muda mesmo
// se a lista for reordenada depois. Os ids aqui são os MESMOS números que já
// existem em toda citação (<sup data-citation data-n="N">) espalhada pela
// cartilha — de propósito, pra não precisar tocar em nenhuma das ~200
// citações já migradas. `order_index` é que define a posição de exibição
// (1º bloco = "1.", 2º = "2." etc.) — ver Referencias.jsx e RichHtml.jsx,
// que resolvem "id estável -> posição atual" em tempo de render.
const REFS = [
...
const { rows } = await pool.query('SELECT id FROM sections WHERE slug = $1', ['referencias']);
if (!rows[0]) {
  console.error('[migrateReferenciasText] seção "referencias" não encontrada no banco.');
  process.exit(1);
}
const sectionId = rows[0].id;

await pool.query(`DELETE FROM blocks WHERE section_id = $1`, [sectionId]);

for (const [i, body] of REFS.entries()) {
  const stableId = String(i + 1);
  await pool.query(
    `INSERT INTO blocks (section_id, order_index, type, heading, body) VALUES ($1, $2, 'paragraph', $3, $4)`,
    [sectionId, i, stableId, body]
  );
}

console.log(`[migrateReferenciasText] ${REFS.length} referências inseridas para "referencias".`);
await pool.end();
